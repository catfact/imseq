...
ffmpeg -y -framerate 0.5 -pattern_type glob -i 'test_frames/*.png' -r 2 -c:v libx264 -crf 0 temp.mp4
# next, apply framerate conversion. interpolation doesn't affect upsamplwed frames, only transitions
ffmpeg -y -i temp.mp4 -vf "framerate=fps=24" -codec:v mpeg4 test_xfade.mp4